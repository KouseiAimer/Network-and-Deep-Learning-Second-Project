"""Plot and summarize ResNet-110 training logs."""
...
def plot_history(history_path: Path, output_path: Path) -> None:
    ...
    fig.suptitle("ResNet-110 on CIFAR-10", fontsize=14)
    fig.tight_layout()
    output_path.parent.mkdir(parents=True, exist_ok=True)
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Plot ResNet-110 training curves")
    parser.add_argument("--history", type=Path, default=root / "results" / "resnet110" / "history.csv")
    parser.add_argument("--output", type=Path, default=root / "results" / "resnet110" / "curves.png")
    parser.add_argument("--summary", type=Path, default=root / "results" / "resnet110" / "summary.json")
