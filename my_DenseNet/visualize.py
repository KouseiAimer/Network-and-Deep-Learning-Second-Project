"""Generate interpretation figures for the final MyDenseNet checkpoint."""
...
def parse_args() -> argparse.Namespace:
    ...
    default_dir = root / "final_result" / "final_se_densenet"
    parser = argparse.ArgumentParser(description="Visualize MyDenseNet predictions and filters")
...
def get_config_value(config: dict[str, object], key: str, default):
    return config.get(key, default)


def build_from_config(config: dict[str, object]) -> torch.nn.Module:
    return build_model(
        depth=int(get_config_value(config, "depth", 190)),
        growth_rate=int(get_config_value(config, "growth_rate", 40)),
        compression=float(get_config_value(config, "compression", 0.5)),
        activation=str(get_config_value(config, "activation", "silu")),
        se_reduction=int(get_config_value(config, "se_reduction", 16)),
        stochastic_depth_rate=float(get_config_value(config, "stochastic_depth_rate", 0.2)),
        classifier_hidden=int(get_config_value(config, "classifier_hidden", 512)),
        classifier_dropout=float(get_config_value(config, "classifier_dropout", 0.2)),
        drop_rate=float(get_config_value(config, "drop_rate", 0.0)),
        transition_dropout=float(get_config_value(config, "transition_dropout", 0.0)),
    )
