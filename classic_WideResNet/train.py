"""Train a strong WideResNet on CIFAR-10."""
...
def parse_args() -> argparse.Namespace:
    ...
    default_output = exp_root / "final_result" / "wrn28_10"

    parser = argparse.ArgumentParser(description="Strong WideResNet for CIFAR-10")
    ...
    parser.add_argument("--activation", choices=["relu", "leaky_relu", "silu", "gelu", "mish"], default="relu")
    parser.add_argument("--se-reduction", type=int, default=0, help="0 disables SE; 16 enables lightweight SE.")
    parser.add_argument("--stochastic-depth-rate", type=float, default=0.0)
    ...
    parser.add_argument("--scheduler", choices=["cosine", "onecycle", "multistep", "none"], default="cosine")
...
def maybe_add_autoaugment(steps: list) -> None:
...
def maybe_add_randaugment(steps: list) -> None:
...
def apply_mix(images: torch.Tensor, targets: torch.Tensor, mode: str, alpha: float):
...
@torch.no_grad()
def evaluate(model: nn.Module, loader: DataLoader, criterion: nn.Module, device: torch.device, max_batches: int = 0):
    model.eval()
    total_loss = 0.0
    total_correct = 0
    total_samples = 0
...
def main() -> None:
    ...
    print(f"Model: WRN-{args.depth}-{args.widen_factor}")
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        train_loss, train_acc = train_one_epoch(model, train_loader, criterion, optimizer, scheduler, device, scaler, args, ema)
        ...
        print(
            ...
            f"lr {lr:.5f} | train loss {train_loss:.4f} acc {train_acc * 100:.2f}% | "
            f"test loss {test_loss:.4f} acc {test_acc * 100:.2f}% | "
            f"best {best_acc * 100:.2f}% | {elapsed:.1f}s"
        )

    title = f"WRN-{args.depth}-{args.widen_factor} on CIFAR-10"
