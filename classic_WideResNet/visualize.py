"""Generate evaluation visualizations for a trained WideResNet."""
...
def parse_args() -> argparse.Namespace:
    ...
    default_dir = root / "final_result" / "wrn28_10"
    parser = argparse.ArgumentParser(description="Visualize WideResNet predictions")
...
def build_from_config(config: dict[str, object]) -> torch.nn.Module:
    return build_model(
        depth=int(cfg(config, "depth", 28)),
        widen_factor=int(cfg(config, "widen_factor", 10)),
        dropout=float(cfg(config, "dropout", 0.3)),
        activation=str(cfg(config, "activation", "relu")),
        se_reduction=int(cfg(config, "se_reduction", 0)),
        stochastic_depth_rate=float(cfg(config, "stochastic_depth_rate", 0.0)),
    )
...
@torch.no_grad()
def collect_predictions(model, loader, device, max_batches: int, max_misclassified: int):
    ...
    for batch_idx, (images, targets) in enumerate(loader):
        ...
        for i in range(images.size(0)):
            ...
            rows.append({"index": sample_index, "label": CLASSES[target], "prediction": CLASSES[pred], "confidence": conf, "correct": target == pred})
            if target == pred:
            ...
            sample_index += 1
    return confusion, rows, correct_conf, wrong_conf, misclassified
...
def main() -> None:
    ...
    confusion, rows, correct, wrong, misclassified = collect_predictions(model, loader, device, args.max_batches, args.max_misclassified)
