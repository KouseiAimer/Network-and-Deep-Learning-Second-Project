"""FinalCNN v2: validated SE-DenseNet-BC architecture for CIFAR-10.

V1 explored a strengthened WideResNet combination but underfit in the full
server run. V2 deliberately returns to the best observed backbone:
SE-DenseNet-BC-190-40 with optional stochastic depth and classifier dropout.
"""
...
class DropPath(nn.Module):
    """Per-sample stochastic depth for newly generated dense features."""
...
class DenseLayer(nn.Module):
    """DenseNet-BC bottleneck layer with optional SE and stochastic depth."""
...
class FinalDenseNetV2(nn.Module):
    """SE-DenseNet-BC for the final high-accuracy CIFAR-10 run."""
...
def describe_model(model: nn.Module) -> dict[str, int | float | str]:
    return {
        ...
        "classifier_hidden": getattr(model, "classifier_hidden", "unknown"),
        "classifier_dropout": getattr(model, "classifier_dropout", "unknown"),
