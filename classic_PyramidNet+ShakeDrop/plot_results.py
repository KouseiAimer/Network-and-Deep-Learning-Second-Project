"""Plot and summarize PyramidNet + ShakeDrop training logs."""
...
def plot_history(
    history_path: Path,
    output_path: Path,
    title: str = "PyramidNet + ShakeDrop on CIFAR-10",
) -> None:
    import matplotlib
    ...
    generalization_gap = [test - train for train, test in zip(train_error, test_error)]
    ...
    axes[1, 1].plot(epochs, generalization_gap, color="tab:purple")
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Plot PyramidNet + ShakeDrop training curves")
    parser.add_argument(
        "--history",
        type=Path,
        default=root / "results" / "pyramidnet110_a270_shakedrop" / "history.csv",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=root / "results" / "pyramidnet110_a270_shakedrop" / "curves.png",
    )
    parser.add_argument(
        "--summary",
        type=Path,
        default=root / "results" / "pyramidnet110_a270_shakedrop" / "summary.json",
    )
    parser.add_argument("--title", type=str, default="PyramidNet-110-a270 + ShakeDrop on CIFAR-10")
